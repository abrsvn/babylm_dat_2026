"""Verify a converted DAT Hugging Face repository against its checkpoint.

Example:
    python scripts/verify_hf_dat.py \
        --hf-dir /path/to/hf-export \
...
from models.attention.dat.config import DatLMConfig  # noqa: E402
from models.attention.dat.lm import DatDecoderLM  # noqa: E402


def _parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Verify a converted DAT HF repo.")
...
def build_live_model(
    ...
    model = DatDecoderLM(DatLMConfig(**config_dict))
...
def main() -> None:
    ...
    hf_logits = hf_output["logits"]  # Expected by the official evaluator.
    ...
    assert torch.allclose(hf_logits, live_logits, atol=1e-4), "HF logits diverge from the checkpoint"
    ...
    assert torch.allclose(hf_hidden, live_hidden, atol=1e-4), "HF AutoModel hidden states diverge"
